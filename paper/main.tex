...
frozen off-the-shelf extractor by design (Section~III-E), but
...
lighting conditions evaluated explicitly (Sections~III-B,
...
condition-dependent and sometimes negative (Section~VI) rather
...
(Section~VI, ``Reproducibility and integrity'').
...
\mathrm{clamp}(\hat{I}_4, 0, 1)$. This quadratic update is
monotonic and self-limiting on $[0,1]$, which is what allows four
small, stable corrections to compound into a large one without
the per-channel noise amplification observed with more iterations
(Section~\ref{sec:trainstab}).
...
single-class transfer-learning workflow used here (Section~V-A)
--- rather than for an accuracy advantage; YOLOv10's reported
gains are modest (1.2--1.4\% mAP) and orthogonal to the
low-light robustness question this paper investigates, so revisiting
this choice is noted as future work (Section~VIII) rather than
argued for here. DARK FACE's bounding
...
shared-threshold problem described in Section~\ref{sec:verifprotocol} was corrected
...
The values are retained exactly as recorded in the supplied manuscript;
they should be cross-checked against the original experimental CSV before
final submission.
The HOG baseline's false-accept rate (FAR) is prohibitively high
across all conditions, confirming that its apparently non-trivial
accuracy masks a system that would freely admit unauthorized
individuals. ArcFace maintains a far lower FAR, which is the
property that actually matters for access control.

\begin{table}[!t]
\caption{False-Accept and False-Reject Rates at Bright and Dark Endpoints}
\label{tab:farfrr}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{FAR (bright)} & \textbf{FAR (dark)} &
\textbf{FRR (bright)} & \textbf{FRR (dark)} \\
\midrule
HOG baseline & 0.54 & 0.87 & 0.31 & 0.44 \\
ArcFace      & 0.06 & 0.18 & 0.09 & 0.29 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
repository \cite{labsecurity}. The repository entry should be checked
before submission to ensure that it contains the final training,
evaluation, configuration, and environment files required to reproduce
