...
\IEEEauthorblockA{[YOUR AFFILIATION / DEPARTMENT / UNIVERSITY]\\
Email: [YOUR EMAIL]}}
...
written by [YOUR NAME]. Specifically: the TD-FALE-GAN
...
\texttt{make\_qualitative\_figure.py}), showing a real DARK FACE
...
in \texttt{ablation\_results\_recognition.csv} (3{,}286 trials per
...
after the fact; the resulting checkpoints
(\texttt{finetune/runs/face\_finetune\_v3/weights/best.pt}) and the
enhancement generator's checkpoint
(\texttt{gan/checkpoints/fale\_generator.pt}) are the exact weights
used to produce every number in Tables~\ref{tab:val}--\ref{tab:farfrr}
and Figure~\ref{fig:qualitative} --- Figure~\ref{fig:qualitative}
was regenerated from these checkpoints specifically for this
revision, not staged from a prior run.
\item \textbf{Raw ablation outputs.} All headline numbers trace to
\texttt{ablation\_results/ablation\_results\_detection.csv} and
\texttt{ablation\_results/ablation\_results\_recognition.csv},
included in the repository; the significance test added in response
to Finding 4 (Appendix~\ref{app:response}) was computed directly
from the trial counts in the latter file.
