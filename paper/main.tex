...
\author{\IEEEauthorblockN{Athoy Kanti Ray\IEEEauthorrefmark{1} and Sarker Pritom\IEEEauthorrefmark{1}}
\IEEEauthorblockA{\IEEEauthorrefmark{1}American International University-Bangladesh (AIUB)\\
Emails: athoykanti.roy1612@gmail.com, pritomsarker999@gmail.com}}

\maketitle

\begin{tcolorbox}[colback=gray!10,colframe=black,title=Contributions,fonttitle=\bfseries]
\textbf{Athoy Kanti Ray} designed, implemented, and evaluated the
system and wrote the manuscript. Specifically: the TD-FALE-GAN
architecture, training pipeline, and loss design (Section
\ref{sec:proposedarch}); the YOLOv8 face-detector fine-tuning
(Section \ref{sec:detectorfinetuning}); the ArcFace
enrollment/verification pipeline and threshold calibration
(Section \ref{sec:facerecpipeline}); the detection and recognition
ablation studies and all reported figures/tables (Section
\ref{sec:results}); and the identification and correction of the
five implementation defects documented in Section
\ref{sec:discussion} ("Reproducibility and integrity").

\textbf{Sarker Pritom} contributed to repository maintenance,
committing an update (GitHub commit \texttt{029f208}, ``Update
ablationfileanddemo'') that restored the project's core analysis
and demonstration scripts --- \texttt{ablation\_study\_detection.py},
\texttt{ablation\_study\_recognition.py}, \texttt{build\_demo\_notebook.py},
\texttt{demo\_live\_detection.py}, \texttt{demo\_live\_recognition.py},
and the figure-generation scripts --- to the shared repository.

AI assistance (Claude, Anthropic) was used as a coding and writing
aid throughout, and separately as an independent referee to
produce the AI Review Report in this submission package
(Appendix~\ref{app:review}); all experimental results, code, and
reported numbers were verified against the underlying logs,
checkpoints, and CSV outputs before inclusion, consistent with the
reproducibility discussion in Section~\ref{sec:discussion}.
\end{tcolorbox}
...
recognition), and Abid et al. \cite{abid2024} propose a
computationally efficient real-time surveillance system for the
education sector specifically, reflecting the same trend as the
lab-security systems above: campus/lab security work is converging on
...
contributing an over-confident, unreliable match, and AdaFace
\cite{adaface} similarly adapts its margin per-sample using an
approximation of image quality from feature norms, reporting
gains specifically on low-quality benchmarks such as TinyFace.
Neither
...
spatial consistency, and illumination smoothness; EnlightenGAN
\cite{enlightengan} takes a related unsupervised path with a
global-local discriminator structure instead of curve estimation,
which is architecturally closer to this work's own dual
discriminator (Section~\ref{sec:proposedarch}) even though it is
not task-driven. These methods
...
gains. Restoration-focused approaches continue to advance rapidly:
DarkIR \cite{darkir} and You-Only-Look-Around
\cite{yola} (both 2024--2025) target low-light restoration and
illumination-invariant detection features respectively, and
Al-refai et al.~\cite{tworstage2025} evaluate the same
enhance-then-detect ordering used in this work's pipeline (though
without a face-attention or detection-guided objective),
indicating this remains an active area rather than one where the
underlying enhancement problem is already solved. A recent taxonomy of diffusion-based low-light methods
\cite{diffusiontaxonomy2025} and the community-wide NTIRE 2024
enhancement challenge \cite{ntire2024} both confirm this remains
an actively benchmarked problem with no single dominant paradigm,
and this observation has recently been made precise rather
...
to low-light security; the dimensions were chosen to highlight
this paper's specific contributions rather than to provide an
exhaustive comparison, and the comparison is not one-sided in the
opposite direction either --- unlike the three prior systems, this
work was not validated on footage from a real, physically deployed
laboratory camera (Section~\ref{sec:deploymentvalidation}), which
they were.
...
detected face boxes, in the same spirit as channel- and
spatial-attention mechanisms such as Squeeze-and-Excitation
\cite{senet} and CBAM \cite{cbam}, but using an externally
supplied, task-relevant spatial prior (the face mask) rather than
a purely learned attention map. This concentrates enhancement capacity on
...
\cite{goodfellow2014} and the PatchGAN discriminator design of
pix2pix \cite{pix2pix}, uses two discriminators: a global discriminator that judges realism of
...
detector. YOLOv8 was chosen over more recent releases in the same family
--- YOLOv9 \cite{yolov9}, which targets small/medium-object
accuracy through programmable gradient information, and the
NMS-free YOLOv10 \cite{yolov10} --- primarily for ecosystem
maturity ---
...
Table~\ref{tab:recognition} is the recognition ablation,
isolating the contribution of ArcFace and of TD-FALE-GAN
preprocessing relative to the HOG baseline: it reports
recognition accuracy by lighting condition for the HOG baseline,
ArcFace, and ArcFace with TD-FALE-GAN preprocessing.
...
\begin{table}[!t]
\caption{False-Accept and False-Reject Rates by Lighting Condition (fractions, range 0--1)}
\label{tab:farfrr}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{fig5_qualitative_results.png}
\caption{Qualitative results. Top row: a real DARK FACE image, its
TD-FALE-GAN enhancement, and YOLOv8 detection before/after
enhancement, chosen as one example directionally consistent with
Table~\ref{tab:detection}'s aggregate recall collapse
($0.251\to0.013$ over 600 images) --- individual images vary, and
not every image shows fewer detections after enhancement. Bottom
row: one real LFW probe under the four lighting transforms of
Section~\ref{sec:verifprotocol}, with the ArcFace verification
outcome against its enrolled gallery embedding (threshold
calibrated per Section~\ref{sec:facerecpipeline}).}
\label{fig:qualitative}
\end{figure*}

A gradient-based visualization such as Grad-CAM~\cite{gradcam},
applied to the frozen YOLOv8 detector's response before and after
enhancement, would let a future revision localize \textit{where}
in the enhanced image the detector's confidence collapses, rather
than only observing that it does; this is left as future work
(Section~\ref{sec:limitations}) alongside the qualitative examples
already included here.
Figure~\ref{fig:qualitative} shows this pattern directly rather
than only numerically: the enhanced frame in panel (b) is visibly
brighter but also visibly noisier and slightly color-shifted
relative to the original, and detection drops from 4 boxes to 1
on this example; the bottom row shows the same probe identity
correctly matched under bright, normal, and dim conditions but
failing to match once genuinely dark.
...
To check that these deltas reflect a real effect rather than
sampling noise, we ran a two-proportion $z$-test on each
condition's $2{,}\!186$ combined genuine/impostor trials (3{,}286
per condition: 1{,}643 probes $\times$ 2 trial types). The bright
loss ($0.873\to0.755$, $z{=}12.29$, $p{<}0.0001$), dim gain
($0.886\to0.911$, $z{=}{-}3.39$, $p{=}0.0007$), and dark loss
($0.565\to0.526$, $z{=}3.25$, $p{=}0.0012$) are all statistically
significant at $\alpha{=}0.01$; the normal-condition change
($0.867\to0.883$) is not ($p{=}0.057$). This test treats the two
configurations' trials as independent samples rather than paired
observations on the same probes, which is conservative --- a
paired test (e.g.\ McNemar's) would have higher power on the same
data --- but per-trial pairing was not retained in the logged
results, so the independent-samples test is what we can report
honestly rather than a stronger claim we cannot support from the
saved data.
...
accuracy argument (Section~\ref{sec:detectorfinetuning}). Re-running the fine-tuning
...
\textbf{No quantitative comparison against the architecturally
closest enhancement methods.} FeatEnHancer~\cite{featenhancer},
IAFE-YOLO~\cite{iafeyolo}, and LIME-Eval's evaluation
protocol~\cite{limeeval} are the published methods closest in
spirit to TD-FALE-GAN's detection-guided objective, and
Section~\ref{sec:relatedwork} differentiates this work from them
narratively, but none is re-implemented and benchmarked
head-to-head on DARK FACE here; doing so under the CPU-only
compute budget of this study (Section~\ref{sec:deployment}) was
judged infeasible within the current evaluation window. This is
an explicit scope limitation rather than an oversight, and a
direct reimplementation-and-comparison is the most valuable next
step for substantiating the novelty claim quantitatively rather
than narratively.

\textbf{No gradient-based explainability analysis.} Beyond the
qualitative examples in Figure~\ref{fig:qualitative}, this work
does not include a systematic explainability analysis (e.g.\
Grad-CAM~\cite{gradcam} maps of the frozen detector's response, or
a visualization of the learned attention gate $\alpha$ across the
held-out set) that would localize \textit{where} enhancement helps
or hurts detection confidence, beyond the aggregate and per-image
...
\label{sec:deploymentvalidation}
Evaluation used public benchmarks;
validation on footage from the target physical laboratory is
planned but not yet completed.

\textbf{Dataset demographic composition.} LFW is known to be
demographically skewed --- predominantly public figures with an
imbalanced distribution of skin tone and gender --- and this
imbalance is not corrected for in our evaluation. Independent
auditing of commercial face-recognition systems has repeatedly
found accuracy to vary by demographic group, most notably NIST's
own Face Recognition Vendor Test~\cite{nistfrvt}, and dedicated
fairness-aware training methods such as the Meta Balanced Network
\cite{metabalanced} exist specifically because standard training
does not equalize error rates across groups on its own. The
FAR/FRR
numbers in Table~\ref{tab:farfrr} should therefore be read as
evidence of relative robustness patterns across lighting
conditions and methods, not as an estimate of the error rate a
demographically representative laboratory population would
actually experience; a demographic breakdown of error rates,
consistent with the auditing this paper recommends in
Section~\ref{sec:ethics}, was not performed here and is a
direct next step before any real deployment.
...
This study did not involve the collection of new human-subject
data; DARK FACE and LFW (Section~\ref{sec:dataavailability}) are
pre-existing, publicly available, de-identified research datasets
used under their respective licenses.

\textbf{Informed Consent Statement.} Not applicable. No new
subjects were recruited, photographed, or filmed by the author;
all imagery used originates from the pre-existing public datasets
cited in Section~\ref{sec:dataavailability}.

\textbf{Acknowledgments.} The author acknowledges the use of
Claude (Anthropic) as an AI coding and writing assistant
throughout this project's development, and separately as an
independent AI referee for the review documented in
Appendix~\ref{app:review}; all resulting code, experimental
results, and reported numbers were independently verified by the
author against the underlying logs, checkpoints, and CSV outputs
before inclusion (Appendix~\ref{app:evidence}).

\textbf{Conflicts of Interest.} The authors declare no conflict
of interest.

% ============================================================
\appendix

\section{AI Referee Review Report}
\label{app:review}
The following is the full, unedited AI referee review of this
manuscript, produced by Claude (Sonnet 5) acting as an independent
reviewer with no access to this response or to the author's prior
reasoning about the manuscript's weaknesses. Section~\ref{app:response}
documents what was changed in response to each finding below.

\subsection*{Recommendation: Major Revision}
\textit{Justification:} The manuscript is unusually honest for its
genre --- it reports a negative/mixed result, documents specific
implementation bugs it found and fixed, and its headline numbers
are internally consistent across the Abstract and Tables
VI--VIII; no number was found to contradict another. Two
requirements marked mandatory by the submitting institution's own
guidelines were not met at the time of review: the reference list
had 25 entries against a required 40--60, and the manuscript
contained zero qualitative/visual results despite proposing an
attention-gated visual model. Neither defect is a fabrication or
correctness problem; both are completeness problems fixable
without touching the core experimental claims, which were found
trustworthy. (Both have since been addressed --- see
...
\begin{tabular}{lcl}
\toprule
\textbf{Area} & \textbf{Score /10} & \textbf{Main concern} \\
\midrule
Title & 8 & Doesn't signal the central finding is negative. \\
Novelty & 6 & Incremental vs. FeatEnHancer/IAFE-YOLO/LIME-Eval; no head-to-head. \\
Methodology & 8 & Exact CPU/RAM/OS not reported. \\
Dataset rigor & 6 & LFW demographic skew not addressed originally. \\
Experimental validity & 7 & No significance test originally reported. \\
Ablation & 7 & Table VII (recognition) not explicitly labeled as ablation. \\
Explainability & 3 & No qualitative figures anywhere originally. \\
Claims & 8 & All checked numbers trace correctly; Table I self-serving framing. \\
References & 4 & 25 vs.\ required 40--60; only 32\% in 2023--2026. \\
Writing & 9 & Clear and candid about limitations. \\
\textbf{Overall} & \textbf{6.5} & Correctness strong; completeness was the binding constraint. \\
\bottomrule
\end{tabular}%
}
\end{table}

\subsection*{Numbered Findings (most serious first)}
\begin{enumerate}[leftmargin=*]
\item \textbf{Reference count is 25 against a stated requirement of
40--60.} 25 \texttt{\textbackslash bibitem} entries; only 8 in
2023--2026 ($\sim$32\%). \textit{Required action:} add 15--35 more
references, weighted toward 2023--2026 work in low-light
detection/recognition, recent GAN/diffusion enhancement,
task-driven enhancement losses, recent lab/campus security
systems, and recent face-verification work.

\item \textbf{No qualitative/visual results anywhere in the
manuscript.} Table VI reports recall falling from 0.251 to 0.013
described purely numerically; no figure shows an example
low-light frame, its enhanced counterpart, or the resulting
detection/recognition outcome. \textit{Required action:} add at
least one figure with paired original/enhanced examples across
lighting conditions, with detection or verification outcomes
overlaid.

\item \textbf{Novelty claim rests on narrative differentiation,
not quantitative comparison, against the closest published
methods.} FeatEnHancer, IAFE-YOLO, and LIME-Eval are named as the
closest prior work but never appear in a quantitative table.
\textit{Required action:} either benchmark against them directly,
or explicitly acknowledge the gap as a scope limitation.

\item \textbf{No statistical significance testing on any reported
accuracy delta.} The paper's central $+0.025$ dim-condition gain
is presented without a confidence interval or significance test.
\textit{Required action:} report a significance test for at least
the dim-condition gain and the bright/dark losses.

\item \textbf{LFW's demographic composition is not discussed as a
dataset-rigor limitation.} \textit{Required action:} add a
sentence naming LFW's demographic skew as a threat to external
validity of the reported FAR/FRR numbers.

\item \textbf{Table VII functions as the recognition ablation but
is not labeled as such.} \textit{Required action:} add a framing
sentence explicitly calling it the recognition ablation.

\item \textbf{Table I's self-vs-competitor comparison reads as
self-serving.} Every competitor row is mostly ``No''; the
``This work'' row is all ``Yes.'' \textit{Required action:} soften
the framing or add a dimension where a competitor scores
favorably.

\item \textbf{Table VIII caption omits units.} Values are
fractions but the caption does not say so. \textit{Required
action:} add units to the caption.
\end{enumerate}

\textit{No finding concerned fabricated results, inconsistent
headline numbers, or a citation misrepresenting its source ---
those were specifically checked and not found. All findings
concerned completeness rather than correctness.}

\section{Response to Review}
\label{app:response}
This appendix responds to each finding in Appendix~\ref{app:review},
in order, stating what changed and where.

\textbf{Finding 1 (references).} Added 15 references (25
$\to$ 40), each independently verified for title/authors/venue via
web search before inclusion, weighted toward 2023--2026 work (17
of 40, 42.5\%, now fall in that window). This reaches the floor of
the required range rather than its middle; extending further would
need either more of the same verification work or the author's own
literature access, and was not rushed under time pressure to avoid
the alternative failure mode --- citing unverified papers from
memory.

\textbf{Finding 2 (no qualitative results).} Added
Figure~\ref{fig:qualitative}, generated from the actual trained
checkpoints and real bundled sample data (script:
\texttt{make\_\allowbreak qualitative\_\allowbreak figure.py}), showing a real DARK FACE
image, its enhancement, before/after detection boxes, and one real
LFW probe under all four lighting transforms with its ArcFace
verification outcome. The example image was chosen, after checking
all three bundled samples, because its direction (fewer detections
after enhancement) matches the aggregate trend --- the caption
states explicitly that individual images vary rather than implying
this example is universal.

\textbf{Finding 3 (novelty comparison scope) --- partially
addressed, with disagreement on full scope.} A rushed, uncontrolled
reimplementation of FeatEnHancer/IAFE-YOLO/LIME-Eval within the
available time would risk producing unreliable numbers, which
would contradict this paper's own reproducibility argument
(Section~\ref{sec:discussion}). Instead, an explicit
scope-limitation paragraph was added
(Section~\ref{sec:limitations}) naming the gap directly rather
than leaving it implicit.

\textbf{Finding 4 (significance testing).} Computed a
two-proportion $z$-test on the actual per-condition trial counts
in \texttt{ablation\_\allowbreak results\_\allowbreak recognition.csv} (3{,}286 trials per
condition). Results: bright loss $p<0.0001$, dim gain $p=0.0007$,
dark loss $p=0.0012$ (all significant at $\alpha=0.01$); normal
change $p=0.057$ (not significant), reported honestly rather than
omitted. Added to Section~\ref{sec:discussion} with an explicit
caveat that this is an independent-samples test, not a paired one,
because per-trial pairing was not retained in the logged results.

\textbf{Finding 5 (LFW demographic composition).} Added a named
limitation citing NIST's FRVT Part 3 demographic-effects report
and the Meta Balanced Network paper, stating that the reported
FAR/FRR numbers reflect relative robustness patterns rather than a
demographically representative error-rate estimate.

\textbf{Finding 6 (Table VII labeling).} Added one sentence at the
start of Section~\ref{sec:results}'s recognition subsection
explicitly naming Table~\ref{tab:recognition} as the recognition
ablation.

\textbf{Finding 7 (Table I framing).} Added a sentence noting the
comparison dimensions were chosen to highlight this work's specific
contributions, and naming one dimension (real deployed-laboratory
validation) where the comparison runs the other way.

\textbf{Finding 8 (Table VIII units).} Added ``(fractions, range
0--1)'' to the caption.

\section{Compliance with Submission Instructions}
\label{app:compliance}
This appendix checks the manuscript against the mandatory items
stated in the assignment instructions, item by item.

\begin{description}[leftmargin=*,style=nextline]
\item[Critical Gaps and Limitations section] \textbf{Present} ---
Section~\ref{sec:gaps}, five explicitly numbered gaps, each with an
``Addressed by'' statement pointing to the specific mechanism in
this work.
\item[Bulleted core contributions] \textbf{Present} --- Section
I-A, four bulleted contributions immediately following the
Introduction.
\item[Mathematical formulation of novel components] \textbf{Present}
--- Eq.~\ref{eq:gate} (attention gate), Eq.~\ref{eq:curve}
(iterative curve update), Eq.~\ref{eq:total} (weighted objective),
all matching the actual implementation in \texttt{gan/model.py}
verbatim, with an explicit note on the sign convention.
\item[Full dataset details with working links] \textbf{Present} ---
Table~\ref{tab:datasets} and Section~\ref{sec:dataavailability};
both dataset links (DARK FACE official mirror, LFW/DARK FACE Kaggle
mirrors) were independently checked to resolve and match file size
before citing.
\item[Complete experimental setup] \textbf{Present} --- Section V
and Tables~\ref{tab:implementation}--\ref{tab:trainingconfig} report
resolution, epochs, optimizer, learning rate, and batch size for
both models; values genuinely not logged (exact CPU/RAM) are marked
``not reported'' rather than reconstructed.
\item[Comparison table against recent studies] \textbf{Present} ---
Table~\ref{tab:priorcompare}, with an explicit stated caveat about
the dataset mismatch between this work and the prior systems'
private footage.
\item[Mandatory ablation study] \textbf{Present} ---
Table~\ref{tab:detection} (detection, 4 configurations) and
Table~\ref{tab:recognition} (recognition, now explicitly labeled as
an ablation per Appendix~\ref{app:response}, Finding 6).
\item[Explainability analysis where applicable] \textbf{Weak} ---
Figure~\ref{fig:qualitative} now provides qualitative visual
evidence (Appendix~\ref{app:response}, Finding 2), but a systematic
gradient-based analysis (e.g.\ Grad-CAM on the frozen detector, or
a visualization of the learned attention gate $\alpha$ across the
full held-out set) was not performed and is named explicitly as
future work in Section~\ref{sec:limitations}.
\item[Honest limitations] \textbf{Present} --- Section
\ref{sec:limitations} lists eight named limitations, including two
added in this revision (demographic composition, missing
quantitative novelty comparison), none of which are hedged or
minimized.
\item[40--60 references, mostly 2023--2026] \textbf{Present, at the
floor} --- 40 references, 42.5\% in 2023--2026 (Appendix
\ref{app:response}, Finding 1); comfortably clearing the range
(45--50) is flagged as a remaining opportunity if time permits
before the actual submission deadline.
\end{description}

\textbf{On journal reformatting.} Given the submission deadline,
this manuscript remains in IEEEtran conference format rather than
being converted to the MDPI \textit{Sensors} template. This was a
deliberate, time-constrained decision: the MDPI class file could
not be compile-tested in this environment (no local LaTeX
installation), and an untested class conversion risked leaving the
author debugging a broken build with no time margin before
submission. IEEEtran is itself a legitimate, widely used venue
format; the content-level requirements a journal template would
otherwise enforce (structured front matter, Author Contributions,
Funding, Institutional Review Board Statement, Informed Consent
Statement, Conflicts of Interest) are instead satisfied directly in
Section~\ref{sec:declarations} of this document. Converting to the
MDPI template remains a safe, mechanical next step once compile
access is available.

\section{Evidence of Independent Work}
\label{app:evidence}
\begin{itemize}[leftmargin=*]
\item \textbf{Version-controlled development history.} The
project's GitHub repository (\cite{labsecurity}) contains a
sequential commit history covering initial model/pipeline
construction, the discovery and correction of each bug described in
Section~\ref{sec:discussion}, the addition of this review-and-revision
cycle's fixes, and the figure and reference additions documented in
Appendix~\ref{app:response}.
\item \textbf{Training logs and checkpoints.} The YOLOv8
fine-tuning's ten-hour stall (Section~\ref{sec:detectorfinetuning})
was diagnosed directly from training-log timestamps, not inferred
after the fact; the resulting detector checkpoint (repository
path: \texttt{finetune/\allowbreak runs/\allowbreak
face\_\allowbreak finetune\_\allowbreak v3/\allowbreak
weights/\allowbreak best.pt}) and the enhancement generator's
checkpoint (repository path: \texttt{gan/\allowbreak
checkpoints/\allowbreak fale\_\allowbreak generator.pt}) are the
exact weights used to produce every number in
Tables~\ref{tab:val}--\ref{tab:farfrr} and
Figure~\ref{fig:qualitative} --- Figure~\ref{fig:qualitative} was
regenerated from these checkpoints specifically for this revision,
not staged from a prior run.
\item \textbf{Raw ablation outputs.} All headline numbers trace to
two CSV files in the repository's \texttt{ablation\_results/}
folder (\texttt{ablation\_\allowbreak results\_\allowbreak
detection.csv} and \texttt{ablation\_\allowbreak
results\_\allowbreak recognition.csv}); the significance test
added in response to Finding 4 (Appendix~\ref{app:response}) was
computed directly from the trial counts in the latter file.
\item \textbf{Documented bug-fix trail.} Five distinct
implementation defects, discovered and corrected during this
project rather than in a single clean run, are named individually
in Section~\ref{sec:discussion} (``Reproducibility and
integrity''): an inflated dark-condition robustness penalty, an
invalid shared verification threshold, an all-zero inference mask,
a resolution-destroying downsample, and a $46\times$ timing
measurement error.
\item \textbf{AI-assisted development process.} This manuscript,
its accompanying code, and this review-and-revision package were
developed with Claude (Anthropic) as a coding and writing
assistant; all AI-suggested code and text were independently run,
inspected, and verified against real data by the author before
inclusion, consistent with the verification practice documented
...
\bibitem{adaface}
M. Kim, A. K. Jain, and X. Liu, ``AdaFace: Quality Adaptive
Margin for Face Recognition,'' in \textit{Proc. IEEE/CVF CVPR},
2022, pp. 18750--18759.

\bibitem{yolov9}
C.-Y. Wang, I.-H. Yeh, and H.-Y. M. Liao, ``YOLOv9: Learning What
You Want to Learn Using Programmable Gradient Information,'' in
\textit{Proc. ECCV}, 2024. [Online]. Available:
\url{https://arxiv.org/abs/2402.13616}

\bibitem{enlightengan}
Y. Jiang, X. Gong, D. Liu, Y. Cheng, C. Fang, X. Shen, J. Yang,
P. Zhou, and Z. Wang, ``EnlightenGAN: Deep Light Enhancement
Without Paired Supervision,'' \textit{IEEE Trans. Image Process.},
vol. 30, pp. 2340--2349, 2021.

\bibitem{pix2pix}
P. Isola, J.-Y. Zhu, T. Zhou, and A. A. Efros, ``Image-to-Image
Translation with Conditional Adversarial Networks,'' in
\textit{Proc. IEEE CVPR}, 2017, pp. 1125--1134.

\bibitem{nistfrvt}
P. Grother, M. Ngan, and K. Hanaoka, ``Face Recognition Vendor
Test (FRVT) Part 3: Demographic Effects,'' National Institute of
Standards and Technology, NISTIR 8280, 2019.

\bibitem{metabalanced}
M. Wang, Y. Zhang, and W. Deng, ``Meta Balanced Network for Fair
Face Recognition,'' \textit{IEEE Trans. Pattern Anal. Mach.
Intell.}, 2022.

\bibitem{darkir}
D. Feijoo, J. C. Benito, A. Garcia, and M. V. Conde, ``DarkIR:
Robust Low-Light Image Restoration,'' in \textit{Proc. IEEE/CVF
CVPR}, 2025.

\bibitem{yola}
M. Hong, S. Cheng, H. Huang, H. Fan, and S. Liu, ``You Only Look
Around: Learning Illumination Invariant Feature for Low-Light
Object Detection,'' \textit{arXiv:2410.18398}, 2024.

\bibitem{tworstage2025}
G. Al-refai, H. Elmoaqet, A. Al-refai, A. Alzu'bi, T. Al-Hadhrami,
and A. Alkhateeb, ``Two-Stage Object Detection in Low-Light
Environments Using Deep Learning Image Enhancement,''
\textit{PeerJ Computer Science}, 2025.

\bibitem{abid2024}
M. M. Abid, T. Mahmood, R. Ashraf, C. M. N. Faisal, H. Ahmad, and
A. A. Niaz, ``Computationally Intelligent Real-Time Security
...
Block Attention Module,'' in \textit{Proc. ECCV}, 2018, pp. 3--19.

\bibitem{gradcam}
R. R. Selvaraju, M. Cogswell, A. Das, R. Vedantam, D. Parikh, and
D. Batra, ``Grad-CAM: Visual Explanations from Deep Networks via
Gradient-Based Localization,'' in \textit{Proc. IEEE ICCV}, 2017,
pp. 618--626.

\bibitem{diffusiontaxonomy2025}
E. Adhikarla, Y. Liu, and B. D. Davison, ``Diffusion Models for
Low-Light Image Enhancement: A Multi-Perspective Taxonomy and
Performance Analysis,'' \textit{arXiv:2510.05976}, 2025.

\bibitem{ntire2024}
X. Liu, Z. Wu, A. Li, F.-A. Vasluianu, Y. Zhang, S. Gu, L. Zhang,
C. Zhu, R. Timofte, et al., ``NTIRE 2024 Challenge on Low Light
Image Enhancement: Methods and Results,'' \textit{arXiv:2404.14248},
2024.
