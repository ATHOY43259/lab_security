...
computed directly from the genuine/impostor trials underlying
Table~\ref{tab:recognition}. The HOG baseline's false-accept rate
(FAR) is prohibitively high in both bright and dark conditions
(0.608 and 0.542 respectively), confirming that its apparently
non-trivial accuracy masks a system that would freely admit
unauthorized individuals. ArcFace maintains a far lower FAR in
both conditions (0.217 bright, 0.021 dark), which is the property
that actually matters for access control. Its false-reject rate,
however, rises sharply from 0.038 (bright) to 0.848 (dark): in
extreme darkness, ArcFace's failure mode shifts from wrongly
\textit{accepting} impostors toward wrongly \textit{rejecting}
genuine users -- the safer direction for a security system to
fail in, though it comes at a real usability cost a deployment
should budget for.
...
\midrule
HOG baseline & 0.608 & 0.542 & 0.848 & 0.898 \\
ArcFace      & 0.217 & 0.021 & 0.038 & 0.848 \\
\bottomrule
\end{tabular}%
}
\end{table}
